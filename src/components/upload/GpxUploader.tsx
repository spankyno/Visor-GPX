...
import { ANONYMOUS_MAX_TRACKS } from "@/lib/plans";
...
export function GpxUploader({ compact = false }: GpxUploaderProps) {
  ...
  const handleFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      if (!isSignedIn && existingCount + files.length > ANONYMOUS_MAX_TRACKS) {
        setError(
          `Sin registrarte puedes cargar hasta ${ANONYMOUS_MAX_TRACKS} archivos GPX a la vez. Crea una cuenta gratis para guardar más rutas y consultarlas cuando quieras.`
        );
        return;
      }
    ...
    [addTracks, existingCount, isSignedIn, setError, setLoading]
  );
