...
interface TracksState {
  ...
  setTrackRemoteInfo: (
    id: string,
    info: { remoteId?: string | null; shareToken?: string | null }
  ) => void;
...
export const useTracksStore = create<TracksState>((set, get) => ({
  ...
  setTrackRemoteInfo: (id, info) =>
    set((state) => ({
      tracks: state.tracks.map((t) =>
        t.id === id
          ? {
              ...t,
              remoteId: info.remoteId !== undefined ? info.remoteId ?? undefined : t.remoteId,
              shareToken: info.shareToken !== undefined ? info.shareToken : t.shareToken,
            }
          : t
      ),
    })),
